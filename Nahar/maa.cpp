...
{
long ph,age;
char name[20],add[50],email[30],con[50],date[22],dep[40];
}list;
char search[56],name[99];
FILE *fp, *ft;
int i,n,z,l,found;
main()
{ 
...
{
printf("\nName\t: %s\nAge\t: %ld\nPhone\t: %ld\nAddress\t: %s\nEmail\t: %s\nConsultant: %s\nDate: %s\nDepartment: %s\n",list.name,list.age,list.ph,list.add,list.email,list.con,list.date,list.dep);
found++;
}
}
fclose(fp);
...
{
printf("\nName\t: %s\nAge\t: %ld\nPhone\t: %ld\nAddress\t: %s\nEmail\t: %s\nConsultant: %s\nDate: %s\nDepartment: %s",list.name,list.age,list.ph,list.add,list.email,list.con,list.date,list.dep);
found++;
if (found%4==0)
{
...
scanf("%[^\n]",name);
while(fread(name,sizeof(list.name),1,fp)==1)
{
